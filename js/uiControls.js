...
function initUI() {
  ...
  if (headerControls) {
    headerControls.addEventListener("click", () => {
      ...
      if (precisionControl) {
        precisionControl.style.display = areHeaderControlsVisible
          ? "block"
          : "none";
      }
      ...
      if (angleModeSidebar) {
        const isSection2 = !document
          .getElementById("section1")
          ?.classList.contains("active");
        if (isSection2) {
          angleModeSidebar.style.display = areHeaderControlsVisible
            ? "block"
            : "none";
        }
      }
...
function displayBetweenMode(resultsElement) {
  ...
  const angleBetween =
    Math.abs(signedAngleRounded) > Math.abs(unsignedAngleRounded)
      ? Math.round((360 - unsignedAngleRounded) * 100) / 100
      : unsignedAngleRounded;
  const uy =
    u.y === 0 ? fmt(0) : u.y < 0 ? `+${fmt(Math.abs(u.y))}` : `-${fmt(u.y)}`;
  const vy =
    v.y === 0 ? fmt(0) : v.y < 0 ? `+${fmt(Math.abs(v.y))}` : `-${fmt(v.y)}`;
  const crs =
    cross === 0
      ? fmt(0)
      : cross < 0
        ? `+${fmt(Math.abs(cross))}`
        : `-${fmt(cross)}`;
  resultsElement.textContent = `Vector u = ${fmt(u.x)}, ${uy}
  |u| = ${fmt(uMag)}
Vector v = ${fmt(v.x)}, ${vy}
  |v| = ${fmt(vMag)}
Dot product = ${fmt(dot)}
  ...
  const crs =
    cross === 0
      ...
      : cross < 0
        ? `+${fmt(Math.abs(cross))}`
        : `-${fmt(cross)}`;
  resultsElement.textContent = `Vector u:
